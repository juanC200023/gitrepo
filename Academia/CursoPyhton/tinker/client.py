...
def client_program():
    ...
    users_widget = Button(window, padx=5, pady=5, text="Listar usuarios", command=lambda: list_users_request(client_socket))
    users_widget.pack(side=RIGHT, padx=5)

    exit_widget = Button(window, padx=5, pady=5, text="Salir", command=lambda: exit_request(client_socket, username, window))
